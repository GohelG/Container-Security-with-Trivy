------------------------------
## title: "Docker Vulnerability Scanning Lab with Trivy"
## description: "A practical guide to building an intentionally vulnerable Flask Docker container, scanning it for security flaws using Aqua Security Trivy, and implementing secure remediations via CI/CD pipelines."
## version: "1.1.0"
...
------------------------------
## 🛡️ Core Concepts## Primary Docker Security Risks
...
------------------------------
## ⚡ Tool Installation## 1. Docker Setup
...
------------------------------
## 🛠️ Project Setup## Directory Structure
...
# Install vulnerable Python dependenciesCOPY requirements.txt .RUN pip install --no-cache-dir -r requirements.txt
COPY . .
EXPOSE 5000
CMD ["python", "app.py"]

------------------------------
## 📦 Build & Run## 1. Assemble the Target Image
docker build -t flask-vuln-app .

## 2. Launch the Application Container
docker run -d -p 5000:5000 flask-vuln-app

## 3. Manage and Stop the Container
